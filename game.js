(() => {
  ...
  const ui = {
    ...
    ultimateCutin: $('#ultimateCutin'), ultimateCutinPortrait: $('#ultimateCutinPortrait'), ultimateCutinImage: $('#ultimateCutinImage'), ultimateCutinHeader: $('#ultimateCutinHeader'), ultimateCutinMode: $('#ultimateCutinMode'), ultimateCutinName: $('#ultimateCutinName'), ultimateCutinQuote: $('#ultimateCutinQuote'),
  ...
  function ensureImage(image) {
    if(!image||image.repairRequested)return image;
    image.repairRequested=true;image.src=image.repairSource;return image;
  }

  function ensureSheet(record) {
    if(!record||record.requested)return record;
    record.requested=true;record.image.src=record.source;return record;
  }
  ...
  for (const [name, source] of Object.entries(PLAYER_IMAGE_SOURCES)) {
    playerImages[name] = createLazyImage(source);
  }
  ensureImage(playerImages.normal);
  ...
  const MOTION_FRAME_INDEX = {
    idle:0, lookLeft:1, lookRight:2, blink:3, step1:4, step2:5, alert:6, stretch:7, yawn:7,
  ...
  for (const [name, source] of Object.entries({
    ...
    mechaMonkey:'./enemy_mecha_monkey.png'
  })) {
    enemyImages[name] = createLazyImage(source);
  }
  ...
  for (const [name, source] of Object.entries({
    ...
    swordPoseImages[name] = createLazyImage(source);
  }

  function preloadPlayerMode(modeName) {
    ensureImage(playerImages[modeName]||playerImages.normal);
  ...
  const BOSS_CUTIN_DATA={
    ...
    shark:{source:'./assets/cutins/boss_shark_tsunami.webp',mode:'ABYSS MECHA SHARK // PRESSURE MAX',name:'ABYSSAL LEVIATHAN',quote:'深海ごと喰らい尽くす！！',className:'boss-shark'},
    gorilla:{source:'./assets/cutins/boss_gorilla_cataclysm.webp',mode:'JUNGLE MECHA GORILLA // RAGE MAX',name:'JUNGLE CORE CATASTROPHE',quote:'大地ごと叩き潰す！！',className:'boss-gorilla'}
  };
  const DARK_CUTIN_DATA={
    normal:{source:'./assets/cutins/dark_feni_chaos.webp',mode:'DARK FENI // 黒翼ノーマル',name:'CHAOS FEATHER ECLIPSE',className:'boss-dark'},
    leak:{source:'./assets/cutins/dark_feni_leak.webp',mode:'DARK FENI // 漏電モード',name:'PURPLE-RED VOLT DOMAIN',className:'boss-dark boss-dark-leak'},
    brokenLcd:{source:'./assets/cutins/dark_feni_lcd.webp',mode:'DARK FENI // 液晶割れモード',name:'BROKEN FRAME EXECUTION',className:'boss-dark boss-dark-lcd'},
    darkMuscle:{source:'./assets/cutins/dark_feni_muscle.webp',mode:'DARK FENI // ゴリマッチョモード',name:'DARK CONTINENT BREAKER',className:'boss-dark boss-dark-muscle'},
    board:{source:'./assets/cutins/dark_feni_board.webp',mode:'DARK FENI // 基板モード',name:'MIRROR CIRCUIT LEGION',className:'boss-dark boss-dark-board'}
  };
  const GRAVITY = 1800;
  const MAX_FALL = 920;
  const JUMP_PAD_VELOCITY = -1120;
  const START_TIME = 150;
  const BOSS_ARENA_WIDTH = 3400;
  const BOSS_INTRO_DURATION = 1.8;
  const BOSS_SPECIAL_MAX = 100;
  ...
  function preloadNearbyAssets() {
    ...
    if(bossGate&&player.x>bossGate.x-2500){
      if(boss?.type==='shark')ensureImage(enemyImages.mechaShark);
      else if(boss?.type==='gorilla'){ensureImage(gorillaBossImage);ensureImage(enemyImages.mechaMonkey);}
      else if(boss?.type==='darkFeni')for(const modeName of['normal','battery','lcd','muscle','king'])preloadPlayerMode(modeName);
      else ensureImage(bossImage);
    }
  }

  const STAGES = [
    { id: '1-1', name: 'スマホ修理商店街', theme: 'city', width: 10600, time: 215, music:'cityRush' },
    { id: '1-2', name: '連続ピット工場', theme: 'city', width: 9800, time: 200, music:'pitRun' },
    { id: '1-3', name: '地下ケーブル迷宮', theme: 'underground', width: 10400, time: 210, music:'underground' },
    { id: '1-4', name: 'クラウド空中回廊', theme: 'sky', width: 10800, time: 220, music:'sky' },
    { id: '1-5', name: 'キング基板・決戦', theme: 'boss', width: 9200, time: 240, bossType: 'titan', music:'fortress', bossMusic:'bossTitan', bossPhaseMusic:'bossTitan2' },
    { id: '1-6', name: '地下迷宮', theme: 'underground', width: 13200, time: 300, maze: true, music:'maze' },
    { id: '1-7', name: '深海リペア海域', theme: 'sea', width: 14000, time: 320, water: true, music:'sea' },
    { id: '1-8', name: '圧壊ウォール・エスケープ', theme: 'factory', width: 11600, time: 210, chaseWall: true, music:'factory' },
    { id: '2-5', name: '深海メカシャーク決戦', theme: 'sea', width: 11000, time: 280, water: true, bossType: 'shark', music:'deepSea', bossMusic:'bossShark', bossPhaseMusic:'bossShark2' },
    { id: '2-6', name: '機鋼ジャングル・獣王決戦', theme: 'jungle', width: 12800, time: 300, bossType: 'gorilla', music:'jungle', bossMusic:'bossGorilla', bossPhaseMusic:'bossGorilla2' },
    { id: '3-1', name: '黒炎の鏡像・ダークフェニ決戦', theme: 'dark', width: 9800, time: 330, bossType: 'darkFeni', music:'darkApproach', bossMusic:'darkFeni', bossPhaseMusic:'darkFeni2', finale:true }
  ];
  ...
  function configureStage() {
    ...
    WORLD_WIDTH = stage.width; WORLD_HEIGHT = stage.maze ? 1180 : stage.finale ? 920 : 820; stageTheme = stage.theme;
    ...
    const enemySets = {
      ...
      jungle: ['mechaMonkey', 'toolMech', 'battleDrone'],
      dark: ['battleDrone', 'boardTrooper', 'jetMech']
    };
    ...
    for (let section = 0; section < sectionCount; section += 1) {
      ...
      const themeLift = stageTheme === 'sky' ? -70 : stageTheme === 'underground' ? 25 : stageTheme==='jungle' ? -12 : 0;
    ...
    if (stage.bossType === 'titan' || stage.bossType === 'gorilla' || stage.bossType === 'darkFeni') {
      ...
      staticPlatforms.push(
        {x:arenaX+420,y:FLOOR_Y-125,w:300,h:20,oneWay:true,bossArena:true},
        {x:arenaX+850,y:FLOOR_Y-255,w:290,h:20,oneWay:true,bossArena:true},
        {x:arenaX+1320,y:FLOOR_Y-380,w:320,h:20,oneWay:true,bossArena:true},
        {x:arenaX+1830,y:FLOOR_Y-250,w:300,h:20,oneWay:true,bossArena:true},
        {x:arenaX+2350,y:FLOOR_Y-135,w:310,h:20,oneWay:true,bossArena:true},
        {x:WORLD_WIDTH-520,y:FLOOR_Y-290,w:280,h:20,oneWay:true,bossArena:true}
      );
      itemBlueprints.push(['muscle',arenaX+470,FLOOR_Y-185]);
      if(stage.bossType==='gorilla')itemBlueprints.push(['battery',arenaX+820,FLOOR_Y-185]);
      if(stage.bossType==='darkFeni')itemBlueprints.push(['battery',arenaX+320,FLOOR_Y-185],['screen',arenaX+1420,FLOOR_Y-440]);
    }
    if (stage.maze) {
    ...
    if(stage.finale){
      enemyBlueprints.length=0;
      transformBlueprints.splice(0,transformBlueprints.length,[WORLD_WIDTH*.22,FLOOR_Y-235],[WORLD_WIDTH*.43,FLOOR_Y-265]);
      coinBlueprints.splice(0,coinBlueprints.length,...Array.from({length:18},(_,index)=>[520+index*250,FLOOR_Y-160-(index%3)*42]));
      const arenaX=WORLD_WIDTH-BOSS_ARENA_WIDTH;
      itemBlueprints.push(
        ['staminaCola',WORLD_WIDTH*.31,FLOOR_Y-190],
        ['staminaCola',arenaX+820,FLOOR_Y-185],
        ['staminaCola',arenaX+2520,FLOOR_Y-185]
      );
    }
    if(stage.bossType){
      const arenaX=WORLD_WIDTH-BOSS_ARENA_WIDTH;
    ...
    vines=stageTheme==='jungle'?[
      {x:WORLD_WIDTH*.16,y:95,length:440,phase:.2},{x:WORLD_WIDTH*.31,y:65,length:505,phase:1.7},{x:WORLD_WIDTH*.48,y:105,length:430,phase:3.1},
      {x:WORLD_WIDTH*.63,y:55,length:515,phase:4.4},{x:WORLD_WIDTH-BOSS_ARENA_WIDTH+760,y:70,length:470,phase:5.6},{x:WORLD_WIDTH-1120,y:55,length:510,phase:2.5}
    ]:[];
  }
  ...
  const ENEMY_PROFILES = {
    ...
    mechaMonkey: { behavior:'hopper', attack:'pulse', alternate:'chargeDash', w:82, h:86, speed:104, hp:2 }
  };
  ...
  function placePlayerSafely(preferredX,preferredY,options={}) {
    ...
    player.dropTimer=0;player.downHeld=false;player.crouching=false;player.vine=null;
  ...
  function spawnRandomWarpGate(force=false) {
    if(bonusStage?.active||warpGate||STAGES[currentStage].finale||boss?.intro)return false;
    const arenaLimit=boss?WORLD_WIDTH-BOSS_ARENA_WIDTH-320:WORLD_WIDTH-380;
    const candidates=staticPlatforms.filter((floor)=>!floor.ceiling&&!floor.bossArena&&floor.w>=330&&floor.x>520&&floor.x<arenaLimit).map((floor)=>{
      const x=floor.x+Math.max(95,Math.min(floor.w-120,floor.w*(.28+Math.random()*.44)));
      return {floor,x,y:floor.y-116};
    }).filter(({x,y})=>{
      const zone={x:x-90,y:y-40,w:180,h:155};
      return (force||Math.abs(x-player.x)>Math.max(520,viewportWidth*.55))&&!hazards.some((hazard)=>overlap(zone,hazard))&&
        !breakables.some((wall)=>wall.alive&&overlap(zone,wall))&&!enemies.some((enemy)=>enemy.alive&&overlap(zone,enemy));
    });
    if(!candidates.length){warpSpawnTimer=4;return false;}
    const point=candidates[Math.floor(Math.random()*candidates.length)];
    warpGate={x:point.x-42,y:point.y,w:84,h:116,life:24,phase:Math.random()*Math.PI*2,opening:1,cooldown:.65};
    sound('warpOpen');say('BONUS WARP 出現！\n紫のゲートへ飛び込め！');return true;
  }

  function enterBonusStage() {
    if(!warpGate||bonusStage?.active)return false;
    const bonusWidth=1640,bonusHeight=780,floorY=650;
    bonusStage={active:true,width:bonusWidth,height:bonusHeight,floorY,timer:16,entryLock:.8,collected:0,
      returnX:player.x,returnY:player.y,returnFacing:player.facing,returnCameraX:cameraX,returnCameraY:cameraY,
      returnMusic:boss?.active?bossMusicName(boss.phase):stageMusicName(),
      platforms:[{x:0,y:floorY,w:bonusWidth,h:160},{x:250,y:510,w:250,h:22,oneWay:true},{x:610,y:400,w:270,h:22,oneWay:true},{x:980,y:500,w:250,h:22,oneWay:true},{x:1275,y:365,w:230,h:22,oneWay:true}],
      coins:[],exit:{x:1510,y:floorY-124,w:82,h:124}};
    for(let index=0;index<30;index++){
      const lane=index%5,group=Math.floor(index/5),x=190+group*230+lane*34,y=floorY-92-(lane%3)*58-(group%2)*55;
      bonusStage.coins.push({x,y,collected:false,phase:index*.47});
    }
    warpGate=null;player.x=110;player.y=floorY-player.h;player.previousY=player.y;player.vx=0;player.vy=0;player.grounded=true;player.jumpCount=0;player.vine=null;
    cameraX=0;cameraY=0;document.body.classList.add('bonus-stage');window.RepairHeroSound?.transition?.('bonus',.22);sound('warpEnter');say('BONUS STAGE!!\n16秒でコインを集めろ！');return true;
  }

  function exitBonusStage(reason='RETURN') {
    if(!bonusStage?.active)return false;
    const saved=bonusStage;bonusStage=null;document.body.classList.remove('bonus-stage');
    placePlayerSafely(saved.returnX,saved.returnY,{allowAir:!!STAGES[currentStage].water,avoidBoss:true});
    player.facing=saved.returnFacing;player.invincible=Math.max(player.invincible,1);cameraX=saved.returnCameraX;cameraY=saved.returnCameraY;
    warpGate=null;warpSpawnTimer=42+Math.random()*20;window.RepairHeroSound?.transition?.(saved.returnMusic,.22);sound('warpExit');
    say(`${reason}\nBONUS COIN ×${saved.collected}`);return true;
  }

  function updateBonusFx(dt) {
    dust.forEach((particle)=>{particle.x+=particle.vx*dt;particle.y+=particle.vy*dt;particle.vy+=130*dt;particle.life-=dt;});dust=dust.filter((particle)=>particle.life>0);
    sparks.forEach((particle)=>{particle.x+=particle.vx*dt;particle.y+=particle.vy*dt;particle.vy+=320*dt;particle.life-=dt;});sparks=sparks.filter((particle)=>particle.life>0);
  ...
  function updateBonusStage(dt) {
    const room=bonusStage;if(!room?.active)return;
    room.timer=Math.max(0,room.timer-dt);room.entryLock=Math.max(0,room.entryLock-dt);
    updatePlayer(dt);updateBonusFx(dt);
    for(const coin of room.coins){coin.phase+=dt*5;if(coin.collected)continue;const box={x:coin.x-14,y:coin.y-14,w:28,h:28};if(overlap(player,box)){const previousCoins=player.coins;coin.collected=true;room.collected+=1;player.coins+=1;player.score+=scoreValue(180);sound('coin');triggerCoinSpeed(previousCoins);}}
    if(player.y>room.height+50){player.x=110;player.y=room.floorY-player.h;player.vx=0;player.vy=0;player.grounded=true;player.invincible=.7;}
    if(room.entryLock<=0&&overlap(player,room.exit))return void exitBonusStage('BONUS CLEAR!!');
    if(room.timer<=0)return void exitBonusStage('TIME UP');
    const target=Math.max(0,Math.min(room.width-viewportWidth,player.x-viewportWidth*.33));cameraX+=(target-cameraX)*Math.min(1,dt*8);cameraY+=(Math.max(-80,player.y-viewportHeight*.58)-cameraY)*Math.min(1,dt*6);
    updateHud();
  }

  function updateWarpGate(dt) {
    if(bonusStage?.active)return;
    if(!warpGate){warpSpawnTimer-=dt;if(warpSpawnTimer<=0)spawnRandomWarpGate();return;}
    warpGate.life-=dt;warpGate.phase+=dt*4;warpGate.cooldown=Math.max(0,warpGate.cooldown-dt);
    if(warpGate.life<=0){warpGate=null;warpSpawnTimer=26+Math.random()*20;return;}
    if(warpGate.cooldown<=0&&overlap(player,warpGate))enterBonusStage();
  }
  ...
  function resetGame() {
    ...
    const duelHealth=STAGES[currentStage].finale?7:3;
    ...
    items = itemBlueprints.map(([type,x,y]) => {
      ...
      return {type,x,y:floor?floor.y-55:y,collected:false,phase:x/40,respawns:type==='staminaCola'&&STAGES[currentStage].finale,respawnTimer:0};
    });
    confetti = [];
    ...
    const desiredTransformCount=STAGES[currentStage].finale?4:3;
    const finaleModes=['battery','lcd','muscle','king'];
    transformItems = transformSpawnPoints.filter((_,index)=>index%Math.max(1,Math.floor(transformSpawnPoints.length/desiredTransformCount))===0).slice(0,desiredTransformCount).map(({x,y},index) => ({ x, y, w:64, h:70, type:STAGES[currentStage].finale?finaleModes[index%finaleModes.length]:initialModes[index%initialModes.length], collected:false, active:true, warning:0, phase:x/50 }));
    if(STAGES[currentStage].bossType==='titan'&&transformItems.length)transformItems[Math.min(1,transformItems.length-1)].type='king';
    transformSpawnTimer = STAGES[currentStage].finale?8+Math.random()*4:20+Math.random()*15; transformHistory = transformItems.slice(-2).map(item=>item.type);
    modeParticles = [];
    ...
    projectiles=[]; wingShots=[];shockwaves=[]; combatFx=[]; kingClones=[];darkClones=[]; bossIntro=0; hitStop=0; bossDefeated=false; goalUnlocked=!boss;
  ...
  function hideNotice() {
    ui.notice.classList.remove('show','speech','ultimate-speech','dark-boss-speech');
    activeNoticePose=null;
  }

  const CUTIN_PRESENTATION_CLASSES=['battery','lcd','muscle','king','boss','boss-titan','boss-shark','boss-gorilla','boss-dark','boss-dark-leak','boss-dark-lcd','boss-dark-muscle','boss-dark-board'];

  function resetUltimateCutinPresentation() {
    if(!ui.ultimateCutin)return;
    ui.ultimateCutin.classList.remove(...CUTIN_PRESENTATION_CLASSES);
    ui.ultimateCutinPortrait?.classList.remove('hidden');
    ui.ultimateCutinImage?.classList.add('hidden');
    ui.ultimateCutinQuote?.classList.add('hidden');
    if(ui.ultimateCutinQuote)ui.ultimateCutinQuote.textContent='';
    if(ui.ultimateCutinHeader)ui.ultimateCutinHeader.textContent='LIMIT BREAK // PHOENIX DRIVE';
  }

  function showUltimateCutin(modeName) {
    if(!ui.ultimateCutin)return;
    resetUltimateCutinPresentation();ui.ultimateCutin.classList.remove('hidden');
  ...
  function currentBossCutinData() {
    if(!boss)return null;
    if(boss.type==='darkFeni')return {...DARK_CUTIN_DATA[boss.darkMode||'normal'],quote:DARK_ULTIMATE_LINES[boss.darkMode||'normal']};
    return BOSS_CUTIN_DATA[boss.type]||null;
  }

  function preloadBossUltimateCutin() {
    const data=currentBossCutinData();if(!data?.source||bossCutinPreloads.has(data.source))return;
    const image=new Image();image.decoding='async';image.src=data.source;bossCutinPreloads.set(data.source,image);
  }

  function showBossUltimateCutin() {
    if(!ui.ultimateCutin)return false;
    const data=currentBossCutinData();if(!data)return false;
    ...
    ui.ultimateCutinPortrait?.classList.add('hidden');
    if(ui.ultimateCutinImage){ui.ultimateCutinImage.src=data.source;ui.ultimateCutinImage.alt=`${boss.name} 必殺技カットイン`;ui.ultimateCutinImage.classList.remove('hidden');}
    if(ui.ultimateCutinHeader)ui.ultimateCutinHeader.textContent=boss.type==='darkFeni'?'IRREGULAR LIMIT // DARK PHOENIX':'BOSS LIMIT BREAK // DANGER';
  ...
  function say(text, expression=null, expressionMode=playerMode, kind=null) {
    ...
    const speech=!!expression;ui.notice.classList.remove('ultimate-speech','dark-boss-speech');ui.notice.classList.toggle('speech',speech);
  ...
  function playerDamageBox() {
    if(player?.crouching&&!stageIsWater())return {x:player.x+7,y:player.y+54,w:player.w-14,h:player.h-54};
  ...
  function stageIsWater() { return !!STAGES[currentStage]?.water && !bonusStage?.active; }
  function activeWorldWidth() { return bonusStage?.active ? bonusStage.width : WORLD_WIDTH; }
  function activeWorldHeight() { return bonusStage?.active ? bonusStage.height : WORLD_HEIGHT; }
  function allPlatforms() { return bonusStage?.active ? bonusStage.platforms : staticPlatforms.concat(movingPlatforms, fragilePlatforms.filter((p) => p.active)); }
  ...
  function chooseTransformMode() {
    if(STAGES[currentStage].finale){
      const finalePool=['battery','lcd','muscle','king'];
      const last=transformHistory[transformHistory.length-1];
      const available=finalePool.filter((type)=>type!==last);
      const chosen=available[Math.floor(Math.random()*available.length)];transformHistory.push(chosen);transformHistory=transformHistory.slice(-3);return chosen;
    }
  ...
  function scheduleTransformSpawn() {
    ...
    sound('itemWarning'); transformSpawnTimer=STAGES[currentStage].finale?8+Math.random()*5:20+Math.random()*15;
  }
  ...
  function updateUltimateSequence(dt) {
    ...
    if(sequence.airborne){player.y=sequence.anchorY;player.vy=0;player.grounded=false;}
    ...
    if(airborne){player.grounded=false;player.vy=Math.max(-135,Math.min(170,resumeVy*.35));}
    return false;
  }
  ...
  function updateIdleMotion(dt,hasControl,speed) {
    ...
    if(player.idleAction){
      ...
      if(player.idleActionTime>=player.idleActionDuration){player.idleAction=null;player.idleActionTime=0;player.idleTime=0;player.nextIdleAction=4.8+Math.random()*4.2;}
      return;
    }
    if(player.idleTime<player.nextIdleAction)return;
    player.idleAction=Math.random()<.55?'yawn':'stretch';player.idleActionTime=0;
    player.idleActionDuration=player.idleAction==='yawn'?1.75+Math.random()*.35:1.25+Math.random()*.3;
  }

  function vineWorldX(vine,offset=vine.length*.7) {
    const ratio=Math.max(0,Math.min(1,offset/vine.length));
    return vine.x+Math.sin(elapsed*1.35+vine.phase)*(10+ratio*22);
  }

  function nearestClimbableVine() {
    if(stageTheme!=='jungle'||bonusStage?.active)return null;
    const centerX=player.x+player.w/2,centerY=player.y+player.h/2;
    return vines.filter((vine)=>centerY>=vine.y+20&&centerY<=vine.y+vine.length+35&&Math.abs(centerX-vineWorldX(vine,centerY-vine.y))<58)
      .sort((a,b)=>Math.abs(centerX-vineWorldX(a,centerY-a.y))-Math.abs(centerX-vineWorldX(b,centerY-b.y)))[0]||null;
  }

  function updateVineMovement(dt) {
    if(!player.vine&&stageTheme==='jungle'&&input.up){
      const vine=nearestClimbableVine();if(vine){player.vine=vine;player.vineOffset=Math.max(48,Math.min(vine.length-18,player.y+player.h/2-vine.y));player.vineSwing=0;player.vineSwingVelocity=player.vx*.08;player.vx=0;player.vy=0;player.jumpHeld=false;sound('vineGrab');say('ツタをつかんだ！\n▲▼で移動・ジャンプで飛び移れ！');}
    }
    const vine=player.vine;if(!vine)return false;
    const climb=Number(input.down)-Number(input.up),horizontal=Number(input.right)-Number(input.left);
    player.vineOffset=Math.max(42,Math.min(vine.length-16,player.vineOffset+climb*205*dt));
    player.vineSwingVelocity+=horizontal*420*dt-player.vineSwing*6.5*dt;player.vineSwingVelocity*=Math.pow(.12,dt);player.vineSwing+=player.vineSwingVelocity*dt;player.vineSwing=Math.max(-62,Math.min(62,player.vineSwing));
    player.x=vineWorldX(vine,player.vineOffset)+player.vineSwing-player.w/2;player.y=vine.y+player.vineOffset-player.h/2;player.vx=0;player.vy=0;player.grounded=false;player.crouching=false;player.dropTimer=0;
    if(horizontal)player.facing=horizontal;
    if(input.jump&&!player.jumpHeld){const launch=horizontal||Math.sign(player.vineSwingVelocity)||player.facing;player.vine=null;player.vx=launch*430;player.vy=-650;player.jumpCount=1;player.jumpHeld=true;player.jumpPoseTime=.14;player.state='jump';sound('vineJump');for(let i=0;i<effectCount(12,6);i++)sparks.push({x:player.x+player.w/2,y:player.y+player.h/2,vx:(Math.random()-.5)*260,vy:80+Math.random()*160,life:.42,size:3+Math.random()*4});return false;}
    if(!input.jump)player.jumpHeld=false;
    player.state='vine';player.anim+=dt*4;player.walkPhase=(player.walkPhase+dt*2.2)%4;updateCompanions(dt);return true;
  }
  ...
  function updatePlayer(dt) {
    ...
    const groundCrouchIntent=!sequenceLocked&&!stageIsWater()&&input.down&&!input.up&&!input.jump&&player.grounded;
    ...
    if(canDash&&!player.dashSoundLatch){sound('dash');player.dashSoundLatch=true;}
    if(!canDash)player.dashSoundLatch=false;
    ...
    if(wantsDrop&&!player.downHeld&&!stageIsWater()){
    ...
    if (playerMode !== 'king' && !aerialUltimateLock && (!input.jump||sequenceLocked) && player.vy < -220) player.vy += GRAVITY * 1.35 * dt;
    if (player.spin > 0) player.spin = Math.max(0, player.spin - dt * 11);

    const wasGrounded = player.grounded;
    if (playerMode !== 'king' && !stageIsWater() && !aerialUltimateLock) player.vy = Math.min(MAX_FALL, player.vy + GRAVITY * dt);
    if(aerialUltimateLock){player.vx=0;player.vy=0;player.y=player.ultimateSequence.anchorY;player.grounded=false;}
    else if(stageIsWater()){moveAndCollide(player,dt,true);player.grounded=false;player.y=Math.max(92,Math.min(activeWorldHeight()-player.h-30,player.y));}else moveAndCollide(player, dt, true);
    player.x = Math.max(0, Math.min(activeWorldWidth() - player.w, player.x));
    if(!bonusStage?.active&&bossGate?.closed&&boss?.intro&&player.x<bossGate.x+bossGate.w+12){player.x=bossGate.x+bossGate.w+12;player.vx=Math.max(0,player.vx);}
    ...
    if(player.grounded&&['walk','sprint'].includes(player.state)){
      const strideLength=player.state==='sprint'?38:54;
      player.walkPhase=(player.walkPhase+speed*dt/strideLength)%4;
    }
    ...
    if(!bonusStage?.active)updateCompanions(dt);
  }
  ...
  function spawnGorillaMinions(){
    if(boss?.type!=='gorilla'||boss.minionsSpawned)return;
    boss.minionsSpawned=true;boss.guardBroken=false;ensureImage(enemyImages.mechaMonkey);
    const positions=[boss.arenaLeft+360,boss.arenaLeft+760,boss.arenaLeft+1160];
    for(const [index,x] of positions.entries()){
      const minion=makeEnemy(['mechaMonkey',x,FLOOR_Y-86]);minion.bossMinion=true;minion.minionIndex=index;minion.originX=x;minion.attackCooldown=2.4+index*.55;minion.cooldown=1.4+index*.4;enemies.push(minion);
    }
    ui.goalLock.textContent='MINION LOCK ×3';ui.goalLock.classList.remove('unlocked');
  }
  ...
  function beginBossEncounter() {
    ...
    if(boss.type==='titan'||boss.type==='gorilla'||boss.type==='darkFeni')boss.y=FLOOR_Y-boss.h;else boss.y=Math.max(110,Math.min(WORLD_HEIGHT-boss.h-70,player.y-25));
    ...
    sound('bossWarning');
    if(window.RepairHeroSound?.transition)window.RepairHeroSound.transition(bossMusicName(1),.35);else window.RepairHeroSound?.music(bossMusicName(1));
  }

  function bossMoveState() { return boss.type==='shark'?'swim':boss.type==='darkFeni'?'darkChase':'chase'; }

  function bossUltimateAttackName() {
    if(boss.type==='titan')return 'overloadStorm';if(boss.type==='shark')return 'abyssTsunami';if(boss.type==='gorilla')return 'jungleCataclysm';
    if(boss.type==='darkFeni')return {normal:'chaosHunt',leak:'electricField',brokenLcd:'blinkExecution',darkMuscle:'earthRend',board:'mirrorLegion'}[boss.darkMode]||'chaosHunt';return 'overloadStorm';
  }

  function bossUltimateLabel() {
    const labels={overloadStorm:'ZERO-DAY MELTDOWN',abyssTsunami:'ABYSSAL LEVIATHAN',jungleCataclysm:'JUNGLE CORE CATASTROPHE',chaosHunt:'CHAOS FEATHER ECLIPSE',electricField:'PURPLE-RED VOLT DOMAIN',blinkExecution:'BROKEN FRAME EXECUTION',earthRend:'DARK CONTINENT BREAKER',mirrorLegion:'MIRROR CIRCUIT LEGION'};
  ...
  function advanceDarkMode() {
    if(boss?.type!=='darkFeni')return;setDarkBossMode((boss.darkModeIndex+1)%DARK_MODE_ORDER.length);
  }
  ...
  function beginBossAttack(name) {
    ...
    if(boss.usingSpecial){
      ...
      boss.state='cutin';boss.cutinTimer=BOSS_CUTIN_TIME;boss.timer=0;hideNotice();showBossUltimateCutin();
      player.invincible=Math.max(player.invincible,BOSS_CUTIN_TIME+.25);sound(boss.type==='darkFeni'?'darkBossCutin':'bossCutin');
    }
  ...
  function updateTitanBossAttack(dt) {
    ...
    }else if(boss.attackName==='overloadStorm'){
      const x=boss.x+boss.w/2,y=boss.y+boss.h*.32;
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;for(let index=0;index<12;index++){const angle=index*Math.PI*2/12;spawnBossShot({kind:'overload',x,y,w:32,h:38,vx:Math.cos(angle)*(245+(index%2)*55),vy:Math.sin(angle)*(245+(index%2)*55),maxDistance:760});}spawnBossGroundWave(x,-410,'bossWave',880);spawnBossGroundWave(x,410,'bossWave',880);shake=26;sound('bossUltimateImpact');}
      if(boss.attackStep===1&&boss.timer>.3){boss.attackStep=2;for(const offset of[-360,-180,0,180,360])spawnBossShot({kind:'overload',x:boss.targetX+offset,y:Math.max(20,boss.targetY-520),w:30,h:62,vx:0,vy:340,maxDistance:680});shake=Math.max(shake,22);sound('bossShot');}
      if(boss.attackStep===2&&boss.timer>.68){boss.attackStep=3;for(let index=0;index<8;index++){const angle=index*Math.PI/4+Math.PI/8;spawnBossShot({kind:'overload',x,y,w:27,h:34,vx:Math.cos(angle)*325,vy:Math.sin(angle)*325,maxDistance:720});}shake=Math.max(shake,20);sound('bossUltimateImpact');}
      if(boss.timer>1.08)finishBossAttack(1.22);
    }
  }

  function updateGorillaBossAttack(dt) {
    const fromX=boss.x+boss.w/2,fromY=boss.y+boss.h*.35,direction=Math.sign(boss.targetX-fromX)||1;
    if(boss.attackName==='apeCharge'){
      if(!boss.attackFired){boss.attackFired=true;boss.vx=direction*(boss.phase===3?570:boss.phase===2?515:455);sound('bossMelee');}
      if(boss.timer>.72)finishBossAttack(.86);
    }else if(boss.attackName==='armSweep'){
      if(!boss.attackFired){boss.attackFired=true;const range=285,box={x:direction>0?boss.x+boss.w-30:boss.x-range+30,y:boss.y+72,w:range,h:150};combatFx.push({x:direction>0?box.x+box.w:box.x,y:box.y+box.h/2,life:.48,size:175,type:'slash'});if(overlap(player,box))hurt(fromX);sound('bossMelee');}
      if(boss.timer>.48)finishBossAttack(.92);
    }else if(boss.attackName==='scrapThrow'){
      if(!boss.attackFired){boss.attackFired=true;const count=boss.phase>=2?3:2;for(let index=0;index<count;index++){const targetX=boss.targetX+(index-(count-1)/2)*95,targetY=boss.targetY;const aim=aimVelocity(fromX,fromY,targetX,targetY,235+(boss.phase-1)*18);spawnBossShot({kind:'scrap',x:fromX,y:fromY,w:30,h:30,vx:aim.vx,vy:aim.vy-95,gravity:290,maxDistance:610});}sound('bossShot');}
      if(boss.timer>.56)finishBossAttack(.9);
    }else if(boss.attackName==='groundPound'){
      if(!boss.attackFired){boss.attackFired=true;boss.vy=-760-(boss.phase-1)*35;boss.vx=direction*155;boss.grounded=false;}
      if(boss.attackFired&&boss.grounded&&boss.timer>.42){spawnBossGroundWave(fromX,-340,'bossWave',650);spawnBossGroundWave(fromX,340,'bossWave',650);shake=reducedEffects?17:25;sound('stomp');finishBossAttack(1.02);}
    }else if(boss.attackName==='roarPulse'){
      if(!boss.attackFired){boss.attackFired=true;for(let index=-2;index<=2;index++)spawnBossShot({kind:'roarPulse',x:fromX,y:fromY,w:28,h:28,vx:direction*(220-Math.abs(index)*12),vy:index*88,maxDistance:620});shake=15;sound('bossShot');}
      if(boss.timer>.62)finishBossAttack(1.08);
    }else if(boss.attackName==='jungleCataclysm'){
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;for(const velocity of[-480,-360,360,480])spawnBossGroundWave(fromX,velocity,'bossWave',940);for(let index=-2;index<=2;index++)spawnBossShot({kind:'earthChunk',x:fromX,y:fromY-35,w:54,h:48,vx:index*92,vy:-430-Math.abs(index)*32,gravity:760,maxDistance:820});shake=reducedEffects?24:36;sound('bossUltimateImpact');}
      if(boss.attackStep===1&&boss.timer>.34){boss.attackStep=2;for(const offset of[-420,-210,0,210,420]){const targetX=boss.targetX+offset;spawnBossShot({kind:'earthChunk',x:targetX,y:45,w:62,h:54,vx:0,vy:310+Math.abs(offset)*.08,gravity:330,maxDistance:780});}shake=Math.max(shake,24);sound('earthRend');}
      if(boss.attackStep===2&&boss.timer>.72){boss.attackStep=3;spawnBossGroundWave(fromX,-540,'bossWave',980);spawnBossGroundWave(fromX,540,'bossWave',980);for(const offset of[-140,140])spawnBossShot({kind:'earthChunk',x:fromX+offset,y:fromY-60,w:70,h:58,vx:offset*2.2,vy:-470,gravity:690,maxDistance:880});shake=Math.max(shake,reducedEffects?25:40);sound('stomp');}
      if(boss.timer>1.16)finishBossAttack(1.32);
    }
  }
  ...
  function updateSharkBossAttack(dt) {
    ...
    }else if(boss.attackName==='abyssTsunami'){
      const direction=Math.sign(boss.targetX-fromX)||1;
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;for(let lane=-5;lane<=5;lane++)spawnBossShot({kind:'waterBlade',x:fromX,y:fromY,w:42,h:22,vx:direction*(245+Math.abs(lane)*8),vy:lane*68,maxDistance:900});for(const velocity of[-390,390])shockwaves.push({kind:'bossWave',x:fromX,y:fromY,w:38,h:170,vx:velocity,life:2.6,friendly:false,owner:'boss',originX:fromX,maxDistance:920});shake=28;sound('bossUltimateImpact');}
      if(boss.attackStep===1&&boss.timer>.32){boss.attackStep=2;const charge=aimVelocity(fromX,fromY,boss.targetX,boss.targetY,690);boss.vx=charge.vx;boss.vy=charge.vy;combatFx.push({x:fromX,y:fromY,life:.75,size:190,type:'speed'});sound('bossMelee');}
      if(boss.attackStep===2&&boss.timer>.7){boss.attackStep=3;for(let lane=-3;lane<=3;lane++){const edgeX=lane%2?boss.arenaLeft+30:boss.arenaRight-30,edgeY=Math.max(90,Math.min(WORLD_HEIGHT-90,boss.targetY+lane*82));const bladeAim=aimVelocity(edgeX,edgeY,boss.targetX,boss.targetY,315);spawnBossShot({kind:'waterBlade',x:edgeX,y:edgeY,w:40,h:20,...bladeAim,maxDistance:980});}shake=Math.max(shake,24);sound('bossUltimateImpact');}
      if(boss.timer>1.15)finishBossAttack(1.28);
    }
  }

  function spawnDarkBossClones() {
    darkClones=[-1,1].map((side,index)=>({x:boss.x+side*150,y:boss.y+index*55,w:boss.w,h:boss.h,vx:0,vy:0,life:7,shotCooldown:.6+index*.35,phase:index*Math.PI,mode:'board'}));
    sound('darkClones');
  }

  function performDarkBlinkStrike(step=0) {
    const oldX=boss.x,side=step%2===0?-(boss.targetFacing||1):(boss.targetFacing||1),offset=(step-1)*105,targetLeft=boss.targetX-player.w/2+offset;
    boss.x=Math.max(boss.arenaLeft,Math.min(boss.arenaRight-boss.w,targetLeft+side*155));boss.y=Math.max(70,Math.min(FLOOR_Y-boss.h,boss.targetY-boss.h*.58+(step%2?54:-36)));boss.vx=0;boss.vy=0;
    combatFx.push({x:oldX,y:boss.y+boss.h/2,life:.42,size:92,type:'teleport'},{x:boss.x+boss.w/2,y:boss.y+boss.h/2,life:.5,size:125,type:'teleport'});
    const slashX=boss.targetX+offset,slashBox={x:slashX-135,y:boss.targetY-player.h*.7,w:270,h:player.h*1.4};
    if(overlap(playerDamageBox(),slashBox))hurt(boss.x);
    for(const angle of[-.32,.32])spawnBossShot({kind:'darkSlash',x:slashX,y:boss.targetY,w:72,h:24,vx:Math.cos(angle)*side*410,vy:Math.sin(angle)*410,maxDistance:560,life:1.55});
    sound('teleportStrike');shake=Math.max(shake,20+step*3);
  }

  function updateDarkFeniBossAttack(dt) {
    const fromX=boss.x+boss.w/2,fromY=boss.y+boss.h*.42,direction=Math.sign(boss.targetX-fromX)||1;
    if(boss.attackName==='darkFeather'){
      ...
      if(!boss.attackFired){
        ...
        for(let index=-3;index<=3;index++){
          const initial=aimVelocity(fromX,fromY,boss.targetX,boss.targetY+index*64,195+Math.abs(index)*13);
          spawnBossShot({kind:'darkChaos',x:fromX,y:fromY,w:38,h:38,vx:initial.vx,vy:initial.vy,maxDistance:930,life:3.8,turnRate:1.35,homingTime:1.65,phase:index*.9});
        }
        shake=22;sound('chaosHunt');
      }
      ...
      if(boss.attackStep===2&&boss.timer>.5){boss.attackStep=3;performDarkBlinkStrike(2);}
      if(boss.attackStep===3&&boss.timer>.78){boss.attackStep=4;for(const direction of[-1,1])spawnBossShot({kind:'darkSlash',x:boss.targetX,y:boss.targetY,w:88,h:28,vx:direction*470,vy:0,maxDistance:680,life:1.7});sound('bossUltimateImpact');}
      if(boss.timer>1.02)finishBossAttack(1.22);
    }else if(boss.attackName==='darkBlink'){
      if(!boss.attackFired){boss.attackFired=true;const oldX=boss.x,side=-(boss.targetFacing||1),targetLeft=boss.targetX-player.w/2;boss.x=Math.max(boss.arenaLeft,Math.min(boss.arenaRight-boss.w,targetLeft+side*105));boss.y=FLOOR_Y-boss.h;boss.vx=0;boss.vy=0;combatFx.push({x:oldX,y:boss.y+boss.h/2,life:.45,size:88,type:'teleport'},{x:boss.x+boss.w/2,y:boss.y+boss.h/2,life:.5,size:110,type:'teleport'});const slashBox={x:boss.targetX-105,y:boss.targetY-player.h*.62,w:210,h:player.h*1.24};if(overlap(playerDamageBox(),slashBox))hurt(boss.x);sound('teleportStrike');shake=13;}
      if(boss.timer>.42)finishBossAttack(.7);
    }else if(boss.attackName==='electricField'){
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;for(let index=0;index<16;index++){const angle=index*Math.PI*2/16;spawnBossShot({kind:'darkLightning',x:fromX,y:fromY,w:24,h:38,vx:Math.cos(angle)*235,vy:Math.sin(angle)*235,maxDistance:680});}for(const offset of[-300,0,300])spawnBossShot({kind:'darkLightning',x:boss.targetX+offset,y:Math.max(30,boss.targetY-470),w:30,h:64,vx:0,vy:270,maxDistance:660});shake=28;sound('electricField');}
      if(boss.attackStep===1&&boss.timer>.38){boss.attackStep=2;for(let index=0;index<8;index++){const angle=index*Math.PI/4+Math.PI/8;spawnBossShot({kind:'darkLightning',x:fromX,y:fromY,w:28,h:42,vx:Math.cos(angle)*340,vy:Math.sin(angle)*340,maxDistance:720});}sound('bossUltimateImpact');}
      if(boss.attackStep===2&&boss.timer>.75){boss.attackStep=3;spawnBossGroundWave(fromX,-440,'darkVoltWave',820);spawnBossGroundWave(fromX,440,'darkVoltWave',820);shake=Math.max(shake,30);sound('electricField');}
      if(boss.timer>1.08)finishBossAttack(1.26);
    }else if(boss.attackName==='earthRend'){
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;for(let index=-2;index<=2;index++){const targetX=boss.targetX+index*150,aim=aimVelocity(fromX,fromY-40,targetX,boss.targetY,245);spawnBossShot({kind:'earthChunk',x:fromX,y:fromY-40,w:68,h:56,vx:aim.vx,vy:aim.vy-315-Math.abs(index)*35,gravity:650,maxDistance:940});}shake=32;sound('earthRend');}
      if(boss.attackStep===1&&boss.timer>.38){boss.attackStep=2;for(const offset of[-330,0,330])spawnBossShot({kind:'earthChunk',x:boss.targetX+offset,y:35,w:78,h:62,vx:offset*.16,vy:320,gravity:360,maxDistance:820});sound('bossUltimateImpact');}
      if(boss.attackStep===2&&boss.timer>.76){boss.attackStep=3;spawnBossGroundWave(fromX,-520,'bossWave',950);spawnBossGroundWave(fromX,520,'bossWave',950);shake=Math.max(shake,38);sound('stomp');}
      if(boss.timer>1.14)finishBossAttack(1.35);
    }else if(boss.attackName==='mirrorLegion'){
      if(!boss.attackFired){boss.attackFired=true;boss.attackStep=1;spawnDarkBossClones();combatFx.push({x:fromX,y:fromY,life:.9,size:165,type:'teleport'});for(let index=0;index<8;index++){const angle=index*Math.PI/4;spawnBossShot({kind:'darkFeather',x:fromX,y:fromY,w:42,h:23,vx:Math.cos(angle)*285,vy:Math.sin(angle)*285,maxDistance:720});}shake=22;}
      if(boss.attackStep===1&&boss.timer>.42){boss.attackStep=2;for(const offset of[-180,0,180]){const aim=aimVelocity(fromX+offset,fromY-90,boss.targetX,boss.targetY,265);spawnBossShot({kind:'darkChaos',x:fromX+offset,y:fromY-90,w:42,h:42,...aim,maxDistance:820,life:3.1,turnRate:.6,homingTime:.8,phase:offset*.02});}sound('darkClones');}
      if(boss.attackStep===2&&boss.timer>.82){boss.attackStep=3;for(const clone of darkClones)clone.shotCooldown=.02;shake=Math.max(shake,24);sound('bossUltimateImpact');}
      if(boss.timer>1.08)finishBossAttack(1.22);
    }
  }
  ...
  function resolveBossContact() {
    ...
    const body=boss.type==='shark'?{x:boss.x+boss.w*.3,y:boss.y,w:boss.w*.7,h:boss.h}:boss.type==='darkFeni'?{x:boss.x+5,y:boss.y+32,w:boss.w-10,h:boss.h-32}:{x:boss.x,y:boss.y+62,w:boss.w,h:boss.h-62};
  ...
  function updateEnemies(dt) {
    ...
    for (const enemy of enemies) {
      ...
      if(previousAttackCharge>0&&enemy.attackCharge<=0){
        ...
        else if(attack==='bite'||attack==='chargeDash'){enemy.vx=Math.sign(dx||1)*enemy.speed*(attack==='chargeDash'?3.45:3.15);}
  ...
  function defeatEnemy(enemy, style='normal', points=500) {
    ...
    if(enemy.bossMinion&&boss?.type==='gorilla'){
      const remaining=gorillaMinionsAlive();sound('minionDown');
      if(remaining>0){ui.goalLock.textContent=`MINION LOCK ×${remaining}`;say(`メカ小猿 撃破！\n残り ${remaining}体`);}
      else{boss.guardBroken=true;ui.goalLock.textContent='GORILLA ARMOR BREAK';say('MINION SHIELD BREAK!!\nメカゴリラへ攻撃可能！');combatFx.push({x:boss.x+boss.w/2,y:boss.y+boss.h/2,life:1,size:boss.w*.72,type:'gold'});shake=Math.max(shake,18);}
    }
  }
  ...
  function drawWarpGate(gate=warpGate,label='BONUS') {
    if(!gate||!visibleInCamera(gate,150))return;
    ctx.save();ctx.translate(gate.x+gate.w/2,gate.y+gate.h/2);const pulse=1+Math.sin(gate.phase)*.08;
    ctx.scale(pulse,1/pulse);ctx.globalCompositeOperation='screen';ctx.shadowColor='#d340ff';ctx.shadowBlur=32;ctx.strokeStyle='#f4b4ff';ctx.lineWidth=7;
    for(let ring=0;ring<3;ring++){ctx.globalAlpha=.9-ring*.22;ctx.beginPath();ctx.ellipse(0,0,26+ring*9,50+ring*7,Math.sin(gate.phase*.3)*.08,0,Math.PI*2);ctx.stroke();}
    ctx.globalAlpha=.72;const core=ctx.createRadialGradient(0,0,4,0,0,48);core.addColorStop(0,'#fff');core.addColorStop(.22,'#cc55ff');core.addColorStop(1,'#2b003d00');ctx.fillStyle=core;ctx.beginPath();ctx.ellipse(0,0,38,58,0,0,Math.PI*2);ctx.fill();
    ctx.globalCompositeOperation='source-over';ctx.globalAlpha=1;ctx.fillStyle='#fff6ff';ctx.font='900 12px Arial';ctx.textAlign='center';ctx.fillText(label,0,-70);ctx.restore();
  }

  function drawBonusStage() {
    const room=bonusStage;if(!room)return;
    const viewW=Math.max(1280,viewportWidth+120),viewH=Math.max(820,viewportHeight+180),gradient=ctx.createLinearGradient(0,-100,0,viewH);
    gradient.addColorStop(0,'#140023');gradient.addColorStop(.55,'#43206a');gradient.addColorStop(1,'#f0a92e');ctx.fillStyle=gradient;ctx.fillRect(-80,-180,viewW+160,viewH+360);
    ctx.save();ctx.translate(-cameraX,-cameraY);
    for(let star=0;star<36;star++){const x=(star*173)%room.width,y=85+(star*97)%470;ctx.globalAlpha=.35+.45*Math.sin(elapsed*3+star);ctx.fillStyle=star%3?'#fff3a0':'#f08cff';ctx.beginPath();ctx.arc(x,y,2+star%4,0,Math.PI*2);ctx.fill();}
  ...
  function updateObjects(dt) {
    ...
    for(const p of projectiles){
      p.life-=dt;
      if(p.homingTime>0){
        p.homingTime=Math.max(0,p.homingTime-dt);const speed=Math.hypot(p.vx,p.vy)||1,current=Math.atan2(p.vy,p.vx),target=Math.atan2(player.y+player.h/2-p.y,player.x+player.w/2-p.x);
        let delta=(target-current+Math.PI*3)%(Math.PI*2)-Math.PI;const maxTurn=(p.turnRate||1)*dt;delta=Math.max(-maxTurn,Math.min(maxTurn,delta));p.vx=Math.cos(current+delta)*speed;p.vy=Math.sin(current+delta)*speed;
      }
    ...
    for (const item of items) {
      item.phase += dt * 4;
      if(item.collected&&item.respawns){
        item.respawnTimer-=dt;
        if(item.respawnTimer<=0&&boss?.alive){
          const lane=[boss.arenaLeft+620,boss.arenaLeft+1680,boss.arenaRight-650][Math.floor(Math.random()*3)];
          item.x=lane;item.y=FLOOR_Y-58;item.collected=false;item.phase=Math.random()*6;sound('colaSpawn');
          combatFx.push({x:item.x,y:item.y,life:.65,size:70,type:'teleport'});
        }
      }
      if (item.collected || !overlap(player,{x:item.x-18,y:item.y-18,w:36,h:36})) continue;
      item.collected=true; player.score += scoreValue(250); const labels={battery:'HP RECOVER',screen:'LCD +1000',toolbox:'DASH BOOST',fenicoin:'FENI COIN +3000',fire:'INVINCIBLE',muscle:'GORI MACHO MODE!!',staminaCola:'STAMINA COLA!!\nHP・DASH 全回復'};
      if(item.type==='battery') player.hp=Math.min(player.maxHp,player.hp+1);
      ...
      if(item.type==='staminaCola'){player.hp=player.maxHp;player.dash=100;player.boost=Math.max(player.boost,2.5);player.invincible=Math.max(player.invincible,.7);item.respawnTimer=14+Math.random()*7;}
      say(labels[item.type]); sound(item.type==='staminaCola'?'staminaCola':item.type);
  ...
  function updateHud() {
    if (!player) return;
    const fullHearts=Math.max(0,Math.min(player.maxHp,Math.ceil(player.hp)));
    const heartSeparator=player.maxHp>5?'':' ';
    ui.hearts.textContent = [...Array(fullHearts).fill('♥'),...Array(Math.max(0,player.maxHp-fullHearts)).fill('♡')].join(heartSeparator);
  ...
  function drawLegacyBackground() {
    ...
    const palettes={city:['#63cbe6','#d8f1dc','#f7d794'],underground:['#10152d','#36285a','#59463d'],sea:['#087fa9','#35c9dc','#b8f4df'],sky:['#287fd1','#9ce7ff','#fff2bd'],boss:['#451251','#b42d44','#ff9b43'],factory:['#161d2b','#38404b','#6e432f'],jungle:['#03130f','#0b4735','#8aaa42']};
  ...
  function drawOsakaBackdrop(viewW) {
    const stageId=STAGES[currentStage]?.id;if(!['1-1','1-2','1-4','1-8'].includes(stageId))return;
    const baseY=585,span=Math.max(1680,viewW+260),firstX=parallaxWrap(-cameraX*.1,span)-520;
    const label=(text,x,y,size=18,color='#fff4ac')=>{ctx.fillStyle=color;ctx.font=`900 ${size}px "Hiragino Kaku Gothic ProN",Arial`;ctx.textAlign='center';ctx.fillText(text,x,y);};
    const neonSign=(x,y,w,h,text,color)=>{ctx.save();ctx.fillStyle='#07131fdd';ctx.strokeStyle=color;ctx.shadowColor=color;ctx.shadowBlur=20;ctx.lineWidth=5;ctx.fillRect(x,y,w,h);ctx.strokeRect(x,y,w,h);label(text,x+w/2,y+h*.68,Math.max(14,Math.min(24,w/text.length*.88)),'#fff');ctx.restore();};
    const drawTsutenkaku=(x)=>{ctx.save();ctx.translate(x,baseY);ctx.fillStyle='#15283b';ctx.strokeStyle='#72efff';ctx.lineWidth=5;ctx.shadowColor='#41ddff';ctx.shadowBlur=18;ctx.fillRect(-21,-276,42,276);ctx.fillRect(-78,-216,156,25);ctx.fillRect(-58,-310,116,46);ctx.beginPath();ctx.moveTo(-24,-310);ctx.lineTo(0,-390);ctx.lineTo(24,-310);ctx.closePath();ctx.fill();ctx.stroke();for(const side of[-1,1]){ctx.beginPath();ctx.moveTo(side*17,-265);ctx.lineTo(side*65,-18);ctx.stroke();}label('通 天 閣',0,-282,19);ctx.restore();};
    const drawCastle=(x,scale=1)=>{ctx.save();ctx.translate(x,baseY);ctx.scale(scale,scale);ctx.fillStyle='#59616a';ctx.beginPath();ctx.moveTo(-178,0);ctx.lineTo(-142,-58);ctx.lineTo(142,-58);ctx.lineTo(178,0);ctx.closePath();ctx.fill();for(let tier=0;tier<4;tier++){const w=250-tier*45,y=-78-tier*61;ctx.fillStyle=tier%2?'#f2eee0':'#dce9dd';ctx.fillRect(-w/2,y,w,53);ctx.fillStyle='#153d39';ctx.strokeStyle='#51b99e';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-w/2-30,y+7);ctx.lineTo(0,y-30);ctx.lineTo(w/2+30,y+7);ctx.lineTo(w/2+14,y+18);ctx.lineTo(0,y-10);ctx.lineTo(-w/2-14,y+18);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#ffd54b';for(let window=-w/2+28;window<w/2-10;window+=40)ctx.fillRect(window,y+20,10,18);}ctx.fillStyle='#ffd83f';ctx.beginPath();ctx.moveTo(-22,-315);ctx.lineTo(0,-350);ctx.lineTo(22,-315);ctx.closePath();ctx.fill();label('大阪城',0,28,20);ctx.restore();};
    const drawHarukas=(x)=>{ctx.save();ctx.translate(x,baseY);ctx.fillStyle='#173b54';ctx.strokeStyle='#62dcff';ctx.lineWidth=4;ctx.shadowColor='#42d7ff';ctx.shadowBlur=14;ctx.fillRect(-72,-430,144,430);ctx.fillRect(-98,-315,196,315);ctx.fillStyle='#5ed5ef99';for(let row=0;row<12;row++)for(let col=0;col<5;col++)ctx.fillRect(-78+col*34,-398+row*31,17,18);ctx.fillStyle='#d9fbff';ctx.fillRect(-91,-125,182,20);label('あべのハルカス',0,28,18);ctx.restore();};
    const drawUmeda=(x)=>{ctx.save();ctx.translate(x,baseY);ctx.fillStyle='#203241';ctx.strokeStyle='#79e8ff';ctx.lineWidth=4;for(const side of[-1,1])ctx.fillRect(side*52-42,-330,84,330);ctx.strokeRect(-130,-330,260,68);ctx.fillStyle='#79e8ff66';ctx.fillRect(-119,-320,238,46);ctx.beginPath();ctx.arc(0,-296,39,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#4cb5d5aa';for(const side of[-1,1])for(let y=-235;y<-20;y+=38)ctx.fillRect(side*52-27,y,55,17);label('梅田スカイビル',0,28,17);ctx.restore();};
    const drawDanjiri=(x)=>{ctx.save();ctx.translate(x,baseY-6);ctx.fillStyle='#5b2b12';ctx.strokeStyle='#ffc052';ctx.lineWidth=5;ctx.shadowColor='#ff742b';ctx.shadowBlur=15;ctx.fillRect(-130,-145,260,130);ctx.beginPath();ctx.moveTo(-168,-145);ctx.lineTo(-92,-218);ctx.lineTo(92,-218);ctx.lineTo(168,-145);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#cb8d2e';for(let carving=-98;carving<=98;carving+=49){ctx.beginPath();ctx.arc(carving,-92,18,0,Math.PI*2);ctx.stroke();}ctx.fillStyle='#17110c';for(const wheel of[-88,88]){ctx.beginPath();ctx.arc(wheel,0,37,0,Math.PI*2);ctx.fill();ctx.stroke();}ctx.strokeStyle='#f3db9d';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(-164,-40);ctx.lineTo(-350,30);ctx.stroke();for(let lantern=-120;lantern<=120;lantern+=48){ctx.fillStyle='#ffefac';ctx.fillRect(lantern-14,-188,28,38);label('祭',lantern,-160,16,'#d83224');}label('岸和田だんじり',0,39,19);ctx.restore();};
    ctx.save();ctx.globalAlpha=.88;
    if(stageId==='1-1'){
      const canal=ctx.createLinearGradient(0,460,0,baseY+50);canal.addColorStop(0,'#0b7597bb');canal.addColorStop(1,'#041c37dd');ctx.fillStyle=canal;ctx.fillRect(0,470,viewW,150);for(let reflection=0;reflection<14;reflection++){ctx.globalAlpha=.32;ctx.fillStyle=reflection%2?'#ff3d85':'#45eaff';ctx.fillRect(parallaxWrap(reflection*137-cameraX*.03,viewW+120)-50,500+(reflection%4)*20,72,5);}ctx.globalAlpha=.88;
      for(let x=firstX;x<viewW+620;x+=span){drawTsutenkaku(x+250);neonSign(x+470,260,235,86,'道 頓 堀','#ff3c91');neonSign(x+730,360,165,65,'たこ焼','#ffb52f');ctx.save();ctx.translate(x+1010,315);ctx.strokeStyle='#ff3b32';ctx.fillStyle='#b5121d';ctx.lineWidth=8;ctx.shadowColor='#ff4438';ctx.shadowBlur=22;ctx.beginPath();ctx.ellipse(0,0,70,38,0,0,Math.PI*2);ctx.fill();ctx.stroke();for(const side of[-1,1])for(let leg=0;leg<3;leg++){ctx.beginPath();ctx.moveTo(side*45,leg*11-20);ctx.lineTo(side*(95+leg*9),leg*26-33);ctx.stroke();}label('大阪・新世界',0,78,17);ctx.restore();}
      ctx.strokeStyle='#d5eff4';ctx.lineWidth=15;ctx.beginPath();ctx.moveTo(0,475);ctx.quadraticCurveTo(viewW*.5,405,viewW,475);ctx.stroke();ctx.lineWidth=4;for(let rail=0;rail<12;rail++){const x=rail*viewW/11;ctx.beginPath();ctx.moveTo(x,456);ctx.lineTo(x,495);ctx.stroke();}
    }else if(stageId==='1-2'){
      ctx.fillStyle='#0a526d99';ctx.fillRect(0,520,viewW,90);for(let x=firstX;x<viewW+640;x+=span){drawCastle(x+390,1.16);neonSign(x+760,315,285,76,'大 阪 城・城下町','#ffd846');}
    }else if(stageId==='1-4'){
      for(let x=firstX;x<viewW+700;x+=span){drawHarukas(x+300);drawUmeda(x+790);neonSign(x+1010,255,240,75,'OSAKA SKY LINE','#5ee9ff');}
    }else{
      for(let x=firstX;x<viewW+700;x+=span){drawDanjiri(x+390);drawCastle(x+900,.72);neonSign(x+1070,300,245,72,'岸和田・祭街道','#ff7438');}
    }
    ctx.restore();
  }
  ...
  function drawDetailedBackground() {
    ...
    const palettes={city:['#07192b','#226681','#f48a54'],underground:['#050714','#16152b','#3f283a'],sea:['#021a3a','#075e8b','#16adbb'],sky:['#103f83','#3ba8d7','#f5d79c'],boss:['#120518','#571438','#e94a2c'],factory:['#070a11','#202836','#5a3327'],jungle:['#020d0a','#0a3b2b','#7e9b38']};
    ...
    }else if(stageTheme==='jungle'){
      ctx.globalAlpha=.34;ctx.fillStyle='#d8ff8b';ctx.beginPath();ctx.arc(viewW*.76-cameraX*.02,135,74,0,Math.PI*2);ctx.fill();
      for(let layer=0;layer<2;layer++)for(let tree=0;tree<9;tree++){
        const spacing=210+layer*54,x=parallaxWrap(tree*spacing-cameraX*(.06+layer*.11),spacing*9)-spacing,base=640-layer*20,trunk=38+(tree%3)*12;
        ctx.globalAlpha=.62+layer*.13;ctx.fillStyle=layer?'#123d2d':'#071f19';ctx.fillRect(x,130+(tree%2)*55,trunk,base-130);
        ctx.fillStyle=layer?'#276044':'#123527';ctx.beginPath();ctx.arc(x+trunk/2,155+(tree%2)*55,78+layer*15,0,Math.PI*2);ctx.arc(x-38,210+(tree%3)*22,64,0,Math.PI*2);ctx.arc(x+65,218,72,0,Math.PI*2);ctx.fill();
        ctx.strokeStyle=layer?'#76c64e':'#2b6d45';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(x+trunk*.5,145);ctx.bezierCurveTo(x+110,250,x-55,340,x+55,505);ctx.stroke();
      }
      ctx.globalAlpha=.8;ctx.fillStyle='#95ff62';for(let firefly=0;firefly<effectCount(14,8);firefly++){const x=parallaxWrap(firefly*173-cameraX*.11,2422),y=185+(firefly*79%390)+Math.sin(elapsed*2+firefly)*13;ctx.shadowColor='#7dff4d';ctx.shadowBlur=12;ctx.beginPath();ctx.arc(x,y,2.5,0,7);ctx.fill();}ctx.shadowBlur=0;
      ctx.fillStyle='#d9ff91';ctx.font='bold 17px Arial';ctx.fillText('MECHA JUNGLE // BEAST KING DOMAIN',38,140);
  ...
  function drawBackground() {
    ...
    const palettes={city:['#07192b','#286d85','#ef8a55'],underground:['#050714','#18162e','#493143'],sea:['#021a3a','#08668f','#18aeb9'],sky:['#113f82','#49add7','#f1d39d'],boss:['#120518','#63163d','#e6532e'],factory:['#070a11','#242d3a','#63392a'],jungle:['#020d0a','#0a3d2d','#91a83c'],dark:['#05000d','#240536','#6b123d']};
    ...
    if(stageTheme==='city'){
      ...
      ctx.globalAlpha=.32;ctx.fillStyle='#d9ff87';ctx.beginPath();ctx.arc(viewW*.77-cameraX*.02,135,70,0,Math.PI*2);ctx.fill();
      for(let layer=0;layer<2;layer++)for(let tree=0;tree<8;tree++){const spacing=250+layer*45,x=parallaxWrap(tree*spacing-cameraX*(.065+layer*.09),spacing*8)-spacing,base=650-layer*25;ctx.globalAlpha=.7;ctx.fillStyle=layer?'#174832':'#08251b';ctx.fillRect(x,145+(tree%2)*45,42+(tree%3)*8,base-145);ctx.beginPath();ctx.arc(x+22,165+(tree%2)*45,78,0,Math.PI*2);ctx.arc(x-35,225,58,0,Math.PI*2);ctx.arc(x+72,220,66,0,Math.PI*2);ctx.fill();}
      ctx.globalAlpha=.82;ctx.fillStyle='#a6ff68';for(let light=0;light<12;light++){const x=parallaxWrap(light*183-cameraX*.1,2196),y=170+(light*71%390)+Math.sin(elapsed*2+light)*12;ctx.beginPath();ctx.arc(x,y,3,0,Math.PI*2);ctx.fill();}
      ctx.fillStyle='#e7ff9b';ctx.globalAlpha=.95;ctx.font='900 17px Arial';ctx.fillText('MECHA JUNGLE // BEAST KING DOMAIN',38,140);
    }else if(stageTheme==='dark'){
      ctx.globalAlpha=.48;ctx.fillStyle='#d136ff';ctx.beginPath();ctx.arc(viewW*.74-cameraX*.018,150,92,0,Math.PI*2);ctx.fill();ctx.fillStyle='#0a0612';ctx.beginPath();ctx.arc(viewW*.74-cameraX*.018-24,132,92,0,Math.PI*2);ctx.fill();
      for(let tower=0;tower<11;tower++){const x=parallaxWrap(tower*230-cameraX*.11,2530)-150,h=180+(tower*73)%360;ctx.globalAlpha=.72;ctx.fillStyle=tower%2?'#160d26':'#0b0815';ctx.fillRect(x,650-h,175,h);ctx.fillStyle='#d441ff';for(let y=680-h;y<610;y+=48)ctx.fillRect(x+26+(tower%3)*18,y,8,20);}
      ctx.strokeStyle='#ff3a9d';ctx.lineWidth=4;for(let crack=0;crack<5;crack++){const x=parallaxWrap(crack*420-cameraX*.06,2100)-60;ctx.globalAlpha=.42;ctx.beginPath();ctx.moveTo(x,40);ctx.lineTo(x+45,170);ctx.lineTo(x+20,260);ctx.lineTo(x+86,390);ctx.stroke();}
      ctx.globalAlpha=.95;ctx.fillStyle='#f6b7ff';ctx.font='900 17px Arial';ctx.fillText('MIRROR CIRCUIT // DARK FENI DOMAIN',38,140);
    }else if(stageTheme==='factory'){
  ...
  function drawClimbVines() {
    if(stageTheme!=='jungle')return;
    for(const vine of vines){
      const rect={x:vine.x-90,y:vine.y,w:180,h:vine.length+70};if(!visibleInCamera(rect,140))continue;
      ctx.save();ctx.strokeStyle='#3f7f37';ctx.shadowColor='#72ff64';ctx.shadowBlur=12;ctx.lineWidth=13;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(vine.x,vine.y);const endX=vineWorldX(vine,vine.length);ctx.bezierCurveTo(vine.x+46,vine.y+vine.length*.28,vine.x-34,vine.y+vine.length*.68,endX,vine.y+vine.length);ctx.stroke();
      ctx.strokeStyle='#9be35f';ctx.shadowBlur=0;ctx.lineWidth=4;ctx.stroke();ctx.fillStyle='#74c84a';for(let leaf=0;leaf<6;leaf++){const y=vine.y+55+leaf*(vine.length-90)/5,x=vineWorldX(vine,y-vine.y)+(leaf%2?13:-13);ctx.save();ctx.translate(x,y);ctx.rotate(leaf%2?-.55:.55);ctx.beginPath();ctx.ellipse(0,0,14,6,0,0,Math.PI*2);ctx.fill();ctx.restore();}
      const near=player&&!player.vine&&Math.abs(player.x+player.w/2-endX)<105&&Math.abs(player.y+player.h/2-(vine.y+vine.length))<180;
      if(near){ctx.fillStyle='#f1ffd2';ctx.font='900 13px Arial';ctx.textAlign='center';ctx.fillText('▲ ツタをつかむ',endX,vine.y+vine.length+28);}
      ctx.restore();
    }
  }
  ...
  function drawWorld() {
    ...
    darkClones.forEach(drawDarkClone);
    if(boss?.intro && (boss.alive || boss.defeat<(boss.type==='darkFeni'?2.7:.7))&&visibleInCamera(boss,260)) drawBoss();
    ...
    shockwaves.forEach(p=>{
      ...
      if(p.kind==='slash'){
        ...
        const darkVolt=p.kind==='darkVoltWave';ctx.strokeStyle=darkVolt?'#ff305c':p.kind==='rush'?'#fff169':p.friendly?'#ffdc35':'#ff3b29';ctx.shadowColor=darkVolt?'#9b24ff':p.kind==='rush'?'#ff4b16':ctx.strokeStyle;ctx.shadowBlur=darkVolt?30:p.kind==='rush'?35:12;ctx.lineWidth=darkVolt?13:p.kind==='rush'?14:9;
  ...
  function drawBoss(){
    ...
    }else if(boss.type==='gorilla'&&gorillaBossImage.complete&&gorillaBossImage.naturalWidth){
      if(boss.vx>0)ctx.scale(-1,1);const spriteSize=Math.max(boss.w,boss.h)*1.34;
      if(boss.phase>=2&&!reducedEffects)ctx.filter=boss.phase===3?'saturate(1.75) contrast(1.24) hue-rotate(-18deg)':'saturate(1.4) contrast(1.12)';
      if(boss.hit)ctx.globalAlpha=.62+.38*Math.sin(elapsed*50);
      ctx.drawImage(gorillaBossImage,-spriteSize/2,boss.h/2-spriteSize,spriteSize,spriteSize);ctx.filter='none';ctx.globalAlpha=1;
  ...
  function drawScenery() {
    ...
    if(stageTheme==='jungle'){
      const ruinStart=Math.max(520,Math.ceil((left-520)/980)*980+520);for(let x=ruinStart;x<right;x+=980){ctx.fillStyle='#263c2f';ctx.fillRect(x,FLOOR_Y-108,92,108);ctx.strokeStyle='#84c754';ctx.lineWidth=4;ctx.strokeRect(x,FLOOR_Y-108,92,108);ctx.fillStyle='#9cff68';ctx.font='bold 13px Arial';ctx.fillText('BEAST // 03',x+7,FLOOR_Y-72);ctx.fillStyle='#18281f';ctx.beginPath();ctx.arc(x+46,FLOOR_Y-34,21,0,7);ctx.fill();ctx.stroke();}
      ctx.strokeStyle='#75d14e';ctx.lineWidth=9;const vineStart=Math.floor(left/520)*520;for(let x=vineStart;x<right+520;x+=520){ctx.beginPath();ctx.moveTo(x,70);ctx.bezierCurveTo(x+180,210,x-70,340,x+115,505);ctx.stroke();}return;
    }
    if(stageTheme==='factory'){
  ...
  function drawItem(item) {
    ...
    const symbols={battery:'🔋',screen:'📱',toolbox:'🧰',fenicoin:'🔥',fire:'🔥',muscle:'👊',staminaCola:'🥤'};
    ctx.save(); ctx.translate(item.x,item.y+Math.sin(item.phase)*6); ctx.shadowColor='#fff25b'; ctx.shadowBlur=18;
    ctx.fillStyle=item.type==='fenicoin'?'#ffd338':item.type==='staminaCola'?'#62ffb0':'#eaffff'; ctx.beginPath(); ctx.arc(0,0,item.type==='staminaCola'?27:22,0,Math.PI*2); ctx.fill();
    ctx.shadowBlur=0; ctx.font=item.type==='staminaCola'?'30px Arial':'26px Arial'; ctx.textAlign='center'; ctx.fillText(symbols[item.type],0,10);
    if(item.type==='staminaCola'){ctx.fillStyle='#071b24';ctx.fillRect(-34,31,68,15);ctx.fillStyle='#fff';ctx.font='900 9px Arial';ctx.fillText('FULL RECOVER',0,42);}ctx.restore();
  }
  ...
  function currentMotionFrame(renderState=player.state) {
    ...
    if(renderState==='vine')return Math.floor(player.walkPhase||0)%2?'stretch':'alert';
    if(renderState==='hurt'||renderState==='charge')return 'alert';
    if(renderState==='skid')return 'alert';
    if(renderState==='sprint')return Math.floor((player.walkPhase||0)/2)%2?'step2':'step1';
    if(renderState==='walk'){
      const cycle=Math.floor(player.walkPhase||0)%4;
      return cycle===1?'step1':cycle===3?'step2':'idle';
    }
    if(renderState!=='idle')return null;
    if(player.idleAction==='yawn')return 'yawn';
    if(player.idleAction==='stretch')return 'doubleJump';
    return 'idle';
  }
  ...
  function walkFrameBlend(renderState=player.state) {
    ...
    const phase=(((player.walkPhase||0)%sequence.length)+sequence.length)%sequence.length,index=Math.floor(phase),raw=phase-index;
    const blend=.5-.5*Math.cos(raw*Math.PI);
  ...
  function playerVisualPose(renderState=player.state,facing=player.facing,speed=Math.abs(player.vx)) {
    const strideAngle=(player.walkPhase||0)*Math.PI*.5,stride=Math.sin(strideAngle),strideSide=Math.cos(strideAngle),pace=Math.min(1,speed/480);
    let shiftX=0,bob=0,tilt=0,scaleX=1,scaleY=1;
    if(renderState==='idle'){
      bob=Math.sin(elapsed*2.35)*2.4+(player.idleAction==='stretch'?-Math.sin(Math.min(1,player.idleActionTime/player.idleActionDuration)*Math.PI)*3:0);
      tilt=Math.sin(elapsed*1.15)*.012;
    ...
    }else if(renderState==='vine'){
      bob=Math.sin(elapsed*3.2)*1.5;tilt=Math.max(-.16,Math.min(.16,(player.vineSwingVelocity||0)*.002));scaleX=.97;scaleY=1.04;
  ...
  function swordHandAnchor(renderState=player.state) {
    // Coordinates are measured from the matching 196 px motion/state frame,
    // rather than reusing one idle point. This keeps the grip inside Feni's
    // leading hand while running, dashing, taking off and falling.
    const anchors={idle:[69,-28],walk:[38,-30],sprint:[39,-30],skid:[47,-25],dash:[63,27],jump:[60,-32],doubleJump:[66,-17],fall:[47,-26],land:[43,42],crouch:[43,42],vine:[58,-12],wingAttack:[58,-17],special:[58,-17],swordAttack:[58,-18],rushAttack:[54,-15],hurt:[47,-22],clear:[63,-20]};
    const modeOffsets={normal:[0,0],battery:[1,1],lcd:[2,0],king:[2,-2],muscle:[5,3]};
    const base=anchors[renderState]||anchors.idle,offset=modeOffsets[playerMode]||modeOffsets.normal;
    const stateScale=renderState==='dash'?(STATE_MODE_SCALE.dash[playerMode]||1):1;
    return {x:base[0]*stateScale+offset[0],y:base[1]*stateScale+offset[1]};
  }

  function drawHeldSwordOverlay(renderState,gripOnly=false) {
    if(!player.hasSword)return;
    const {x:handX,y:handY}=swordHandAnchor(renderState);
  ...
  function drawFeniSprite(x, y, facing, rotation = 0, alpha = 1, forcedMode = null, forcedState = null) {
    ...
    let currentImage=playerImages[imageKey],resolvedImageKey=imageKey;
    if(!currentImage?.complete||!currentImage.naturalWidth){currentImage=playerImages.normal;resolvedImageKey='normal';}
    const meta=PLAYER_SPRITE_META[resolvedImageKey]||PLAYER_SPRITE_META.normal;
  ...
  function drawPlayerMotionAccents() {
    ...
    if(player.state==='idle'&&player.idleAction==='yawn'){
      const phase=Math.min(1,player.idleActionTime/player.idleActionDuration);ctx.globalAlpha=Math.sin(phase*Math.PI);
      ctx.fillStyle='#e9fbff';ctx.strokeStyle='#17334d';ctx.lineWidth=3;ctx.font='900 18px Arial';ctx.textAlign='center';
      ctx.strokeText('ふぁ〜',player.x+player.w*.82,player.y-14-phase*12);ctx.fillText('ふぁ〜',player.x+player.w*.82,player.y-14-phase*12);
    }else if(player.state==='idle'&&player.idleAction==='stretch'){
      const phase=Math.sin(Math.min(1,player.idleActionTime/player.idleActionDuration)*Math.PI);ctx.globalAlpha=phase*.7;ctx.strokeStyle='#fff0a0';ctx.lineWidth=3;
      for(const side of[-1,1]){ctx.beginPath();ctx.moveTo(player.x+player.w/2+side*34,player.y+18);ctx.lineTo(player.x+player.w/2+side*52,player.y-12);ctx.stroke();}
    }
  ...
  function drawOxygenGear(front=false) {
    if(!stageIsWater())return;
    ctx.save();ctx.translate(player.x+player.w/2,player.y+player.h/2);ctx.scale(player.facing,1);
    if(!front){
      ctx.shadowColor='#4deaff';ctx.shadowBlur=14;ctx.fillStyle='#a9cbd3';ctx.strokeStyle='#eaffff';ctx.lineWidth=3;drawRoundedRect(-49,-18,29,72,12);ctx.strokeRect(-45,-13,21,62);
      ctx.fillStyle='#29b7cb';ctx.fillRect(-45,-2,21,22);ctx.fillStyle='#f5d449';ctx.fillRect(-42,-24,15,9);ctx.strokeStyle='#233c4a';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-31,-9);ctx.quadraticCurveTo(-3,-29,18,-42);ctx.stroke();
      ctx.strokeStyle='#65f4ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-27,-8);ctx.lineTo(15,-16);ctx.moveTo(-26,31);ctx.lineTo(16,22);ctx.stroke();
    }else{
      ctx.fillStyle='#bffaff55';ctx.strokeStyle='#eaffff';ctx.shadowColor='#5ceeff';ctx.shadowBlur=16;ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(27,-38,25,16,-.08,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle='#29dff0';ctx.globalAlpha=.72;ctx.fillRect(13,-42,28,5);ctx.globalAlpha=1;
      for(let bubble=0;bubble<3;bubble++){const bx=55+bubble*11,by=-42-((elapsed*38+bubble*23)%62);ctx.globalAlpha=.45+bubble*.14;ctx.strokeStyle='#d8ffff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(bx,by,3+bubble,0,Math.PI*2);ctx.stroke();}
    }
    ctx.restore();
  }
  ...
  function drawEnemy(enemy) {
    ...
    if(enemy.bossMinion){ctx.fillStyle='#ffef75';ctx.font='900 11px Arial';ctx.textAlign='center';ctx.fillText(`GUARD MONKEY ${enemy.minionIndex+1}`,0,-enemy.h*.72);}
  ...
  if(new URLSearchParams(location.search).has('debug')){
    window.__repairHeroDebug=Object.freeze({
      ...
      attachVine:(index=0)=>{const vine=vines[index];if(!vine)return false;player.vine=vine;player.vineOffset=Math.min(vine.length-20,Math.max(55,vine.length*.72));player.vineSwing=0;player.vineSwingVelocity=0;player.x=vineWorldX(vine,player.vineOffset)-player.w/2;player.y=vine.y+player.vineOffset-player.h/2;player.grounded=false;player.vx=0;player.vy=0;return true;},
      ...
      hitBossMinion:(index=0,amount=99)=>{const target=enemies.filter((enemy)=>enemy.alive&&enemy.bossMinion)[index];return target?damageEnemy(target,amount,'debug',650):false;},
